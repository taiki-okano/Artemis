...
public class AtheneService {
    ...
    public AtheneService(TextSubmissionService textSubmissionService, TextBlockRepository textBlockRepository, TextClusterRepository textClusterRepository,
            TextExerciseRepository textExerciseRepository, TextAssessmentQueueService textAssessmentQueueService, TextPairwiseDistanceRepository textPairwiseDistanceRepository,
     ...
     */
    public void processResult(Map<Integer, TextCluster> clusters, List<AtheneDTO.TextBlockDTO> blocks, List<TextTreeNode> clusterTree, List<List<Double>> distanceMatrix, Long exerciseId) {
        ...
        processClusters(clusters, textBlockMap, clusterTree, distanceMatrix, exerciseId);
     ...
     * @param exerciseId The exerciseId of the exercise the blocks belong to
     * @param clusterTree The cluster tree containing the hierarchicy between cluster
     */
    public void processClusters(Map<Integer, TextCluster> clusterMap, Map<String, TextBlock> textBlockMap, List<TextTreeNode> clusterTree, List<List<Double>> distanceMatrix, Long exerciseId) {
        // Find exercise, which the data belong to
        ...
        TextExercise exercise = optionalTextExercise.get();
        ...
        for (int i = 0; i < distanceMatrix.size(); i++) {
            ...
            for (int j = i + 1; j < distanceMatrix.size(); j++) {
                ...
                textPairwiseDistanceRepository.save(dist);
            }
        }
